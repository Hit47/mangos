...
void recursive_decomposition(stack<string> &s_args, const string args)
{
	...
	// effacement des blancs au début de la chaîne
	while ( arg_dec.substr(0,1) == " " )
		arg_dec.erase(0,1);
	// on place l'index jusqu'à la délimitation de fin d'argument (";")
	...
	// poursuite du traitement de la première chaîne
	--arg_index;
	// effacement des blancs à la fin de la chaine
	while ( arg_dec.substr(arg_index,1) == " " )
	{
		arg_dec.erase(arg_index,1);
		--arg_index;
		if ( arg_index == 0 )
			return; // chaîne vide, pas besoin d'ajouter la chaine à la liste
	}
	s_args.push(arg_dec);
	return;
}
...
void recursive_decomposition(stack<string> &s_args, const char* args)
{
	...
	while ( arg_dec.substr(arg_index,1) == " " )
	{
		arg_dec.erase(arg_index,1);
		--arg_index;
		if ( arg_index == 0 ) return; // Chaîne vide, pas besoin d'ajouter la chaine à la liste
	}
	s_args.push(arg_dec);
	return;
}
...
bool ChatHandler::HandleGMCAttackCommand(const char* args)
{
	...
	// Instanciation du protocole d'échange
